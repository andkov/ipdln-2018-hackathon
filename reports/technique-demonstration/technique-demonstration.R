...
quick_save <- function(g,name){
  ggplot2::ggsave(
    ...
    plot=g,
    device = png,
    path = "./reports/technique-demonstration/prints/1/", # female marital educ poor_healt
...
# ---- model-specification ----------------------------------------
# for the sake of modularization, create a data set that will be used for modeling
ds2 <- ds1 %>% 
  # reorganize the order of variables to match your modeling preference
  dplyr::select_("person_id", "PR", "S_DEAD"
                 ,"age_group"
                 , "female", "marital", "educ3","poor_health", "FOL") %>%
...
# define the model equation for using PROVINCE as a predictor in the model, not stratum
eq_global_string <- paste0(
  "dv ~ -1 + PR + age_group + female + marital + educ3 + poor_health + FOL "
  # "dv ~ -1 + PR + age_group + female + marital + educ3 + poor_health + FOL + ONL"
  # some contingency tables of ONL have small values and produce singularities
)
...
eq_local_string <- paste0(
  ...
  "dv ~ -1      + age_group + female + marital + educ3 + poor_health + FOL"
